...
- `minimal` is a barebones starting point
- `httpbin` is a working reverse proxy example
- `dev` is a full-featured development setup
...
Both are range validated and applied during parsing, improving safety against oversized or malicious headers.
...
Internal CLI modules moved from `cli::conf` to `cli::config`.\ User-facing commands remain unchanged.
...
Header mutation APIs are now gated behind the `wasm` feature.\ They are only available when WASM support is enabled.

### Removal of `rust-embed`

Embedded config templates have been removed.\ Templates are now generated programmatically at runtime, reducing binary size and improving clarity.
