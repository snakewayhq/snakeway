...
- **Listener-level connection controls**
    - **Connection filtering** (CIDR allow/deny, IP family controls, and behavior when the peer address is missing).
    - **Connection rate limiting** to cap new connections per client per window.
    - These are configured under your listener `bind` block, so you can enforce them before requests reach routing and devices.
...
- **Config dump improvements**
    - `snakeway config dump` supports emitting both "spec" (as-written config) and "runtime" (lowered internal representation) to help debug what Snakeway loaded.
...
- **Tests and fixtures refreshed**
    - Expanded integration coverage for newly added devices and network-level controls to reduce regressions in realistic proxy scenarios.

## Upgrading from v0.5.4

- Review any configs that previously relied on "plugin" CLI naming and switch to the WASM device command naming.
