...
| `http2`                           | `object`  | (optional) | HTTP/2 server tuning. Only valid when `enable_http2 = true`. See [HTTP/2 settings](#http2-settings).                                                                                                  |
...
All values must be greater than zero. The two window sizes are capped at 2^31 - 1
