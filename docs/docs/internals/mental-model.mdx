...
<ThemedSvg src="/img/diagrams/mental-model/core-loop.svg" alt="Mental Model Core Loop" />
...
WASM devices currently run after all builtin devices, including the structured logging device. A future release will reorder WASM devices to run before structured logging so that log entries can reflect any mutations made by user-defined devices.
