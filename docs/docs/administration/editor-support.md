...
It underlines the same errors `snakeway config check` reports, completes field names and keyword values, shows
...
- `snakeway.hcl` is served as the entrypoint.
...
gets feedback before its entrypoint exists.
...
1. Install the LSP4IJ plugin, version 2024.2 of the IDE or later.
2. Open the language server settings and click `+` to open the New Language Server dialog. The dialog is also reachable from the LSP console menu.
3. On the Server tab, name the server `Snakeway` and set the command to `snakeway lsp`. The command must be on your `PATH`, or give its absolute path.
...
An unrelated HCL file is left alone, with one note in the LSP console naming it.
...
1. Install an HCL grammar extension, such as HashiCorp's `HCL`, so `.hcl` files carry the `hcl` language id.
...
The client starts the server for every HCL file, and the server's own routing sorts them out: a Snakeway config file
gets its schema, and an unrelated HCL file is left alone with one note in the `Generic LSP Client` output channel.

Any other client that can launch a stdio language server for HCL files works the same way: the command is`snakeway lsp`,
and the file pattern is `*.hcl` under your config directory.

## Limitations

The server checks each file on its own, so a few `snakeway config check` rules stay outside the editor.
Run `snakeway config check` before a deployment, as before.

- Rules that read the whole configuration: a duplicate device across files, a device that requires the identity device,
  a duplicate service name across ingress files, and listener uniqueness.
- Rules that read the host: a certificate file that must exist, or a data directory that must be present, can report
  differently under your editor's working directory.
