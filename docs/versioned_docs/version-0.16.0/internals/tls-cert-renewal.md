...
On startup (and periodically thereafter), the `CertManager` checks each ACME-managed domain.
If a certificate is missing or due to expire within `renew_within_days`, it initiates the ACME renewal flow described above.
